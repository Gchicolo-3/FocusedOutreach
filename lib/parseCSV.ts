...
function generateId(company: string, contact: string): string {
  const raw = `${company}-${contact}`.toLowerCase().replace(/[^a-z0-9]/g, '-');
  return raw;
}
...
function extractBroker(comments: string): string | undefined {
  ...
  const brokerNames = [
    'Peter Shikar', 'Brenden McBride', 'Joe DeVries',
    'Melissa Isman', 'Conor Ryan', 'Alex Dombrowski',
    'Jason Horowitz', 'Tom Chilenski',
  ];
